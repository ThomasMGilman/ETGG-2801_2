...
#jh added these for 2801 fall 2019
#assumes the library is in a folder 
def jh():
    import os
    if os.getenv("PYSDL2_DLL_PATH") == None:
        dname = os.path.dirname(inspect.getfile(inspect.currentframe()))
        if (sys.maxsize > 0xffffffff ):
            os.putenv("PYSDL2_DLL_PATH",os.path.join(dname,"x64"))
        else:
            os.putenv("PYSDL2_DLL_PATH",os.path.join(dname,"x86"))
jh()
